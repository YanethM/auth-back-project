...
const bulkImportStorage = multer.diskStorage({
  destination: function (req, file, cb) {
    const uploadPath = path.join("uploads", "bulk-imports");
  ...
  filename: function (req, file, cb) {
    ...
    const randomString = Math.round(Math.random() * 1e9);
    const ext = path.extname(file.originalname);
    const filename = `bulk-import-${timestamp}-${randomString}${ext}`;
    cb(null, filename);
  },
});

// Filtro para CSV y XLSX únicamente
const bulkImportFileFilter = (req, file, cb) => {
  ...
  cb(
    new Error(
      `Tipo de archivo no permitido. Solo se permiten: CSV, XLSX. Recibido: ${file.mimetype}`
    )
  );
};
...
module.exports = {
  ...
  uploadBulkImport: uploadBulkImport.single("file"),
};